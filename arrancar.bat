...
# CONFIGURACIÓN CENTRALIZADA - GENO.IAFUTURECODED (BLINDADA)
EMAIL_USER = os.environ.get('EMAIL_USER', 'eddiedejesus3011@gmail.com')
EMAIL_PASS = os.environ.get('EMAIL_PASS')  # Removido el texto plano por seguridad táctica
...
def ejecutar_purga_silenciosa():
    if not EMAIL_PASS:
        print("[-] Error en purga: EMAIL_PASS no está configurado en el entorno.")
        return
    try:
